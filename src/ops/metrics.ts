...
class MetricsRegistry {
  ...
  watchTicksTotal = new Map<string, CounterEntry>(); // key = channel|game
  watchErrorsTotal = 0;
  ...
  minutesPerGame = new Map<string, number>();
  minutesPerChannel = new Map<string, number>();
  ...
  setWatching(channelId: string, channelLogin: string, game: string): void {
    if (
  ...
  incWatchTick(channelId: string, channelLogin: string, game: string, minutes = 1): void {
  ...
  incInventoryFetch(): void {
    this.inventoryFetchTotal += 1;
  }

  setCampaigns(total: number, eligible: number): void {
    this.campaignsTotal = total;
    this.eligibleCampaigns = eligible;
  }

  setPubSubConnected(connected: boolean): void {
  ...
  hydrateFromHistory(summary: { perGame: Record<string, number>; perChannel: Record<string, number> }): void {
    for (const [game, mins] of Object.entries(summary.perGame)) {
      const cur = this.minutesPerGame.get(game) ?? 0;
      if (mins > cur) this.minutesPerGame.set(game, mins);
      else if (cur === 0) this.minutesPerGame.set(game, mins);
    }
    for (const [ch, mins] of Object.entries(summary.perChannel)) {
      const cur = this.minutesPerChannel.get(ch) ?? 0;
      if (mins > cur) this.minutesPerChannel.set(ch, mins);
      else if (cur === 0) this.minutesPerChannel.set(ch, mins);
    }
  }
  ...
  render(): string {
    ...
    const addHeader = (name: string, help: string, type: "gauge" | "counter"): void => {
    ...
    if (this.watchingChannelId || this.watchingChannelLogin) {
      lines.push(
        `twitch_drops_watching{channel="${esc(this.watchingChannelLogin)}",channel_id="${esc(this.watchingChannelId)}",game="${esc(this.watchingGame)}"} 1`
      );
    } else {
    ...
    if (this.watchTicksTotal.size === 0) {
      ...
      for (const entry of this.watchTicksTotal.values()) {
        lines.push(
          `twitch_drops_watch_ticks_total{channel="${esc(entry.labels.channel)}",channel_id="${esc(entry.labels.channel_id)}",game="${esc(entry.labels.game)}"} ${entry.value}`
        );
      }
    }
...
try {
  ...
  if (fs.existsSync(dbPath)) {
    try {
      const require = createRequire(import.meta.url);
      const mod = require("node:sqlite") as { DatabaseSync: new (p: string, opts: Record<string, unknown>) => any };
      const db = new mod.DatabaseSync(dbPath, { readonly: true });
      const rowsGame = db.prepare("SELECT game, SUM(minutes_total) as total FROM ticks GROUP BY game").all() as Array<{
        game: string;
        total: number;
      }>;
      const rowsCh = db
      ...
      for (const r of rowsGame) perGame[r.game] = r.total;
      const perChannel: Record<string, number> = {};
      for (const r of rowsCh) perChannel[r.login] = r.total;
    ...
    try {
      ...
      if (raw) {
        ...
        for (const line of raw.split("\n")) {
          try {
            const obj = JSON.parse(line) as { game: string; channelLogin: string; minutesTotal?: number };
...
export function startMetricsServer(opts: MetricsServerOpts): http.Server {
  if (metricsServer) {
