...
import { Channel, canWatchChannel, sortChannelCandidates, shouldSwitchChannel, resolveWatchedChannel } from "../domain/channel.js";
...
export class Miner {
  ...
  async run(options?: { dryRun?: boolean }): Promise<void> {
    ...
    this.maintenance.start(60 * 1000, async () => {
      ...
      if (currentMinute % 5 === 0) {
        const hasClaimable = this.campaigns.some((c) =>
          c.drops.some((d) => d.canClaim || (!d.isClaimed && d.remainingMinutes === 0))
        );
        if (hasClaimable) {
      ...
      if (type === "drop-progress") {
        ...
        if (instanceId && Number.isFinite(minutes)) {
          ...
          if (drop) {
            ...
            const anyUnclaimedAt100 = this.campaigns.some((c) =>
              c.drops.some((d) => !d.isClaimed && d.remainingMinutes === 0)
            );
  ...
  private switchChannel(): void {
    ...
    if (resolved !== this.watchingChannel) {
      logger.info(
        { prevChannel: this.watchingChannel?.login ?? null },
        "Watched channel left the live directory, reselecting"
      );
