...
test("channels without wanted game or dropsEnabled are filtered by canWatchChannel", () => {
  ...
  assert.equal(
    canWatchChannel(
      { id: "1", login: "x", online: true, viewers: 1, gameName: "Other", dropsEnabled: true },
      wanted
    ),
    false
  );
  assert.equal(
    canWatchChannel(
      { id: "2", login: "y", online: true, viewers: 1, gameName: "GameA", dropsEnabled: false },
      wanted
    ),
    false
  );
  assert.equal(
    canWatchChannel(
      { id: "3", login: "z", online: true, viewers: 1, gameName: "GameA", dropsEnabled: true },
      wanted
    ),
    true
  );
