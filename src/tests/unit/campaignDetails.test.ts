...
function campaignsResponse(campaigns: unknown[]) {
  return { data: { currentUser: { dropCampaigns: campaigns } } } as Record<string, unknown>;
}
...
test("merged campaignDetails restore drops so the campaign becomes earnable", () => {
  ...
  const built = buildInventoryFromGqlResponses(emptyInventory, res, {
    ...
    campaignDetails: details as Record<string, Record<string, unknown>>
  });
