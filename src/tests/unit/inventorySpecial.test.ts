...
test("totalRequiredMinutes uses max not sum (upstream parity)", () => {
  ...
  const camp = new DropsCampaign(rawCampaign as any, {}, false);
...
test("canWatchChannel allows special IRL anywhere (upstream b5e1993)", () => {
  const ch = { id: "1", login: "streamer", online: true, viewers: 1000, gameName: "Some Random Game", gameId: "509672", dropsEnabled: true, aclBased: false } as any;
...
test("canWatchChannel allows ACL-based channels regardless of game", () => {
  const ch = { id: "1", login: "streamer", online: true, viewers: 100, gameName: "Random Game", dropsEnabled: true, aclBased: true } as any;
...
test("DropsCampaign parses allowedChannelIds from allow.channels", () => {
  ...
  const camp = new DropsCampaign(raw as any, {}, false);
