...
test("GameDirectory synced to upstream hash + full variables", () => {
  ...
  assert.equal((op.variables as any).limit, 30);
  assert.ok("slug" in op.variables);
  assert.equal((op.variables as any).imageWidth, 50);
  assert.equal((op.variables as any).includeCostreaming, false);
  assert.ok("options" in op.variables);
  const opts = (op.variables as any).options;
  ...
  assert.equal((op.variables as any).sortTypeIsRecency, false);
});
...
test("PlaybackAccessToken operation shape", () => {
  ...
  const v = op.variables as any;
  assert.equal(v.isLive, true); assert.equal(v.isVod, false); assert.equal(v.platform, "web"); assert.equal(v.playerType, "site"); assert.equal(v.vodID, "");
});
...
test("CampaignDetails operation shape", () => {
  ...
  const v = op.variables as any; assert.ok("channelLogin" in v); assert.ok("dropID" in v);
});
...
test("ChannelPointsContext present (future use)", () => {
  ...
  assert.ok(op); assert.equal(op.operationName, "ChannelPointsContext");
...
test("Inventory and Campaigns retain fetchRewardCampaigns:false", () => {
  assert.deepEqual((GQL_OPERATIONS.Inventory.variables as any).fetchRewardCampaigns, false);
  assert.deepEqual((GQL_OPERATIONS.Campaigns.variables as any).fetchRewardCampaigns, false);
});
test("gqlPayload merges variable overrides for GameDirectory", () => {
  const payload = gqlPayload(GQL_OPERATIONS.GameDirectory, { slug: "just-chatting" }) as any;
...
test("gqlPayload for SlugRedirect name resolution", () => {
  const payload = gqlPayload(GQL_OPERATIONS.SlugRedirect, { name: "Just Chatting" }) as any;
