...
import {
  ...
  isMinerLockHeldByLiveProcess,
  minerLockPath,
  releaseMinerLock
} from "../../core/runtime.js";

function withTempHome(): { tmp: string; restore: () => void } {
  const prevHome = process.env.HOME;
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "tdm-lock-test-"));
  process.env.HOME = tmp;
  assert.ok(
    minerLockPath().startsWith(tmp),
    `lock path should follow $HOME override (got ${minerLockPath()})`
  );
  return {
    tmp,
    restore: () => {
