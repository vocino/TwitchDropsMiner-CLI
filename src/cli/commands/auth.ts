...
const importTokenCommand = new Command("import")
  ...
  .action(async (opts) => {
    let raw = opts.token as string | undefined;
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error(`Failed to import token: ${(err as Error).message}`);
      process.exitCode = 1;
    }
  });
...
const importCookieCommand = new Command("import-cookie")
  ...
  .action(async (opts) => {
    let header = opts.cookie as string | undefined;
...
const validateCommand = new Command("validate")
  ...
  .action(async (opts) => {
    ...
    // eslint-disable-next-line no-console
    console.log(
      `Auth state: token=${result.hasToken ? "present" : "missing"}, cookies=${
        result.hasCookies ? "present" : "missing"
      }`
    );
