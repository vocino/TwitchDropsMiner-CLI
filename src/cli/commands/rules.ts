...
export interface Rule {
  if: string; // JS expression: e.g. 'game == "Overwatch" && remaining < 30'
  action: "pin_channel" | "skip" | "prioritize" | "exclude_game";
  game?: string;
  channel?: string;
}
...
export const rulesCommand = new Command("rules")
  ...
  .action(async (opts) => {
    ...
    if (opts.add) {
      ...
      const rules = loadRules();
      rules.push({ if: cond, action: action as Rule["action"] });
      saveRules(rules);
...
export function evaluateRules(rules: Rule[], ctx: { game: string; viewers?: number; remaining?: number; availability?: number; channel?: string; tags?: string[] }): { action?: Rule["action"]; matched?: Rule } {
  for (const rule of rules) {
    try {
      ...
      if (ok) return { action: rule.action, matched: rule };
    } catch {
