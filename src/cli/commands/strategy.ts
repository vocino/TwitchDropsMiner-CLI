...
async function fetchAllCampaigns(): Promise<DropsCampaign[]> {
...
export const calendarCommand = new Command("calendar")
  ...
  .action(async (opts) => {
    ...
    let filtered = campaigns.filter((c) => {
      if (c.endsAt.getTime() < now.getTime()) return false;
      if (c.startsAt.getTime() > horizon.getTime()) return false;
      if (c.drops.length === 0) return false; // skip empty (badges-only) campaigns
      return true;
    });

    if (opts.upcoming) filtered = filtered.filter((c) => c.upcoming);
    if (opts.active) filtered = filtered.filter((c) => c.active && c.eligible);
    ...
    console.log(
      `Calendar — next ${days}d — ${entries.length} campaigns (active ${filtered.filter((c) => c.active).length}, upcoming ${filtered.filter((c) => c.upcoming).length})`
    );
...
export const optimizeCommand = new Command("optimize")
  ...
  .action(async (opts) => {
    ...
    const mode = String((opts as any).mode || "history") as "ending_soonest" | "low_avbl_first" | "history";
    const now = new Date();
    ...
    const scored: Scored[] = earnable.map((c) => {
      ...
      const histMins = (history.perGame[c.gameName] as number) ?? 0;
      let score = 0;
      let reason = "";
    ...
    scored.forEach((s, i) => {
      console.log(
        `${String(i + 1).padStart(3)}  ${s.game.padEnd(20)} ${String(s.remaining).padStart(6)}m ${s.availability.toFixed(2).padStart(6)}  ${String(s.historyMins).padStart(7)}m  ${s.reason}`
      );
    });
...
export const simulateCommand = new Command("simulate")
  ...
  .action(async (opts) => {
    ...
    console.log(
      `Simulate — next ${hours}h — priority ${priority.length ? priority.join(", ") : "(none, using ending_soonest)"}`
    );
    ...
    for (const s of sims) {
      total += s.remaining;
      console.log(
        `${s.game.padEnd(20)} ${String(s.remaining).padStart(4)}m remaining -> completes at +${s.willCompleteInHours}h | claims: ${s.willClaim.join(", ")}`
      );
    }
