...
import { fetchChannelsForWantedGames, parseSlugRedirectResponse, resolveGameSlug } from "../../core/channelService.js";
import { GQL_OPERATIONS } from "../../integrations/gqlOperations.js";
function mockCampaign(name, slug) {
    return { gameName: name, gameSlug: slug };
}
...
test("resolveGameSlug uses campaign slug when resolveSlugs=false", async () => {
    ...
    const slug = await resolveGameSlug("WoW", campaigns, "t", async () => ({ data: { game: { slug: "should-not-use" } } }), { resolveSlugs: false });
    assert.equal(slug, "world-of-warcraft");
});
test("resolveGameSlug falls back to SlugRedirect GQL", async () => {
    const slug = await resolveGameSlug("Just Chatting", [], "t", async (op) => {
        if (op.operationName === GQL_OPERATIONS.SlugRedirect.operationName)
            return { data: { game: { slug: "just-chatting" } } };
        return { data: { game: { streams: { edges: [] } } } };
    }, { resolveSlugs: true });
