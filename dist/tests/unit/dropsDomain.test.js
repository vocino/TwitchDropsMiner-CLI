...
test("TimedDrop preconditionsMet blocks earn until preconditions claimed", () => {
    ...
    const campaign = new DropsCampaign(campaignRaw, {}, true);
    const pre = campaign.timedDrops.get("pre1");
    const chained = campaign.timedDrops.get("dChain");
...
test("TimedDrop canEarnWithin respects preconditions (mirrors upstream _base_earn_conditions)", () => {
    ...
    const campaign = new DropsCampaign(campaignRaw, {}, true);
    const b = campaign.timedDrops.get("b");
    ...
    campaign.timedDrops.get("a").markClaimed();
