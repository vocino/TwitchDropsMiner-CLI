...
export const dropsCommand = new Command("drops")
    ...
    const invResp = (await gqlRequest(GQL_OPERATIONS.Inventory, auth.accessToken));
    const campsResp = (await gqlRequest(GQL_OPERATIONS.Campaigns, auth.accessToken));
    ...
    for (const c of built.campaigns) {
        if (opts.game && !c.gameName.toLowerCase().includes(opts.game.toLowerCase()))
            continue;
        for (const d of c.drops) {
            if (opts.claimable && !d.canClaim)
                continue;
            entries.push({
                game: c.gameName,
                campaignId: c.id,
    ...
    if (opts.json) {
        ...
        if (entries.length === 0) {
            console.log(opts.claimable ? "No claimable drops right now." : "No drops matched.");
            return;
        }
