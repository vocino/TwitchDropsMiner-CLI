...
- **Repo docs:** `docs/parity.md` + `docs/ops/*.md` — deep ops reference. Wiki is the front door for users; repo docs are the source for ops detail. The wiki lives in its own git at `https://github.com/vocino/TwitchDropsMiner-CLI.wiki.git` (branch `master`) — clone that repo to edit wiki pages directly.
